...
public class OkPlant extends BlockFlower implements ITextureProvider
{
    public OkPlant(int par1, int par2)
    {
        super(par1, par2);
    ...
    public ArrayList<ItemStack> getBlockDropped(World world, int x, int y, int z, int metadata, int fortune)
    {
        ...
        if (metadata == 7)
        {
            ret.add(new ItemStack(mod_ReactionCraft.OKBud));
        }
        
        for (int n = 0; n < 10 + fortune; n++)
        {
            if (world.rand.nextInt(15) <= metadata)
            {
                ret.add(new ItemStack(mod_ReactionCraft.OKSeed));
            }
        }
    ...
    {
        return par1 == 7 ? mod_ReactionCraft.AncientFruit.shiftedIndex : -1;
    }
